...
**What's actually a bug:** **7 genuine product bugs** (F1, F5, F7, F12, F14, F15, F18) — **1 high-sev (F15, fixed)**, the rest med — **all 7 now fixed** (F14 was the last, closed 2026-06-22 via mcp #141). Not "~5 / oversold 9×"; the real raw-to-genuine ratio is ~2.6×. **3 of 7 (F1, F5, F14) required cross-surface discovery** a single-surface suite would miss.
...
2. ✅ **BUILT (2026-06-22) — the money canary.** Was: no live cross-surface VALUE assertion; the parity guards prove the *code* is byte-identical but nothing asserted a **UI dollar value == the same MCP tool's value**. Now: deterministic `test/money-canary.test.js` (CI-gated; the REAL shipped UI aggregators — revenue, P&L/COGS, AR aging, tax-by-state — run against committed fixtures grounded in the "Auric & Oak" golden tenant, asserting value+flag parity to the cent; F14/F1 negative-control demonstrated) + live `scripts/qa-spine/money-canary.mjs` (MCP oracle self-check — the W6 matrix, every channel canonical — plus the authed UI cross-check). See [money-canary.md](money-canary.md). This is the class that found F1/F5/F14.
...
**Runner model → deterministic backbone for the gaps; agent as a *periodic cross-surface discovery* tool.** The backbone exists; extend it deterministically (golden seed + money-canary value assertion). The agent's durable, evidenced value is **cross-surface divergence discovery** — 3 of 7 genuine bugs (F1, F5, F14) needed it, and F14 (since fixed, mcp #141) proved a single-surface suite misses this class. Run it **periodically (per major surface change), supervised** — not nightly infra. When it confirms a finding, graduate it to a deterministic test (as F15→`orders-timestamp-sort.test.ts` already did in the mcp repo).
...
3. ✅ **Money canary DONE** — deterministic `test/money-canary.test.js` (CI-gated) + live `scripts/qa-spine/money-canary.mjs`. Covers revenue total, by-channel canonicality (F1), cents-vs-dollars/100x, POS double-count, Timestamp coercion, P&L + the F14 margin-withhold gate, F5 tax-by-state (stateless surfacing), and AR aging — run against committed "Auric & Oak" golden fixtures (test/fixtures/money-canary.*.json); FAILS loudly on divergence (F14/F1 negative-control demonstrated). The genuinely-missing layer is now in place.
