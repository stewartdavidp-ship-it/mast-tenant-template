...
| Parallel sessions merging to main (hot main) | Every merge conflicts your PR on the MAST_MODULES_V line → DIRTY → CI won't even run. Recipe: `git rebase -X theirs origin/main` (safe when your index.html delta is bump-only) + amend (hook re-bumps) + force-push + `gh pr merge --squash --auto` — auto-merge wins the race window |
| Commit touching only docs/generated/** | Path filters mean NO workflow runs → the PR sticks to the old failing check forever. Squash generated-file fixes into the code commit |
| Selector scoped to `#mastSlideOut` | That id does NOT exist — the SO body is `#mastSlideOutBody`, footer `#mastSlideOutFooter`. A scoped collector silently matches nothing (days/certs/skills saved empty across three waves) |
