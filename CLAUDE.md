...
4. **Deploy via `mast-deploy storefront` (canonical) or `mast_hosting` MCP tool (underlying mechanism).** ⚠️ **First read "Deploy Environments & Tiers" below and the canonical [docs/release-model.md](docs/release-model.md) — they govern which deploys need ceremony (prod) and which are pre-authorized (dev).** A deploy uploads code to a Firebase Hosting site. The Cloudflare worker (`runmast-tenant-proxy`) decides whether any given tenant hostname reads from that site — based on the `podRouting` KV entry for the hostname (or the fallback `mast-tenant-shared.web.app`). **Deploying alone does not switch what a tenant serves; you also need the right KV entry.** Job 2b Stage 3b added a pod-aware wrapper:
...
## Deploy Environments & Tiers — read before any deploy

Canonical model: **[docs/release-model.md](docs/release-model.md)**. The essentials:

**Protection scales with blast radius. Pods are NOT equal.** Two tiers:

| Tier | Targets | Ceremony |
|---|---|---|
| **Frictionless** | the **dev pod** = one site `mast-tenant-shared` (sgtest15 + all dev/test tenants); dev KV edits; preview channels | **none — pre-authorized** |
| **Gated** | prod pod deploys (`prod-us-east-1`, `prod-us-west-1`); the `mast-platform-prod` **functions** surface; `runmast.com`; **prod `podRouting` KV edits** | one `yes` confirmation |

- **Dev is pre-authorized.** Do NOT ask the user to deploy or route within the dev pod, do NOT gate on migration scripts, do NOT run "E2E-before-deploy" discipline for dev. It is disposable — iterate freely, fix forward.
- **The project name lies:** the `dev` pod's GCP project is named `mast-platform-prod`. `pod: dev` is NOT production. Don't freeze over the name.
...
- RBAC is one module-anchored model (E6). Permissions are per-module `{view,edit,delete}` triads (shown as an access Level: None/View/Edit/Full) stored on `role.modulePermissions`; high-trust verbs live in `role.sensitiveActions` (declared in `FUNCTION_PERMISSIONS`). Gate module surfaces with **`can(route, 'view'|'edit'|'delete')`** and sensitive verbs with `hasPermission(entity, action)`. There is NO per-entity CRUD map — **never reintroduce `hasPermission('x','create'|'read'|'update'|'delete')`** (the RBAC guard fails CI on it). **Adding a module:** add its route to `SECTION_ROUTES` (it then auto-appears in the permission matrix + sidebar filter + route gate with a section-derived default), gate its write/delete actions with `can()`, and if it's sensitive add a `byModule` exception in `ROLE_DEFAULT_LEVELS`. **Adding a sensitive verb:** one `FUNCTION_PERMISSIONS` entry + a `hasPermission()` gate. Coverage is enforced by `scripts/lint-rbac.js` (PostToolUse hook + CI): every sidebar `data-route` must be in `SECTION_ROUTES` or `UNGATED_ALLOWLIST`. Resolver self-test: `window.__rbacSelfTest()`. Per-user overrides round-trip through the admin-only `updateUserPermissions` CF (ceiling logic in `mast-architecture/functions/rbac-ceiling.js`).
...
- Deploy tiering (protection scales with blast radius). **Dev-pod deploys/routing are pre-authorized — never ask, never gate, never run E2E-before-deploy for dev** (it's the disposable test env; the `mast-platform-prod` project name does NOT make it prod). **Gated tier** (prod pod deploys, the `mast-platform-prod` *functions* surface, `runmast.com`, and **prod `podRouting` KV edits**) requires a single deliberate `yes` and pod-sequential rollout (east-1 → verify → west-1). One gate, one layer (the tool), keyed on target — never stacked. See [docs/release-model.md](docs/release-model.md).
- Concurrency-safe git flow. For any non-trivial build: **always work in an isolated worktree + feature branch** (not the shared checkout — branches alone don't prevent two sessions clobbering each other's uncommitted edits in one working tree). Merge via **PR to `main`** (no fast-forward straight to main); CI auto-deploys dev from `origin/main`, so do NOT hand-deploy dev from a local HEAD. Verify post-merge through `<tenant>.runmast.com` and fix forward. See [docs/release-model.md](docs/release-model.md).
