...
(function () {
  ...
  function placeCheckOrder() {
    ...
    MastDB.set('orders/' + orderKey, orderData).then(function() {
      ...
      // NOTE: Wholesale pay-by-check bypasses submitOrder and any payment webhook,
      // so pass/enrollment provisioning is not triggered here. Wholesale carts
      // typically don't include pass/class items. If that changes, this path
      // needs its own CF to mirror provisionPassesForOrder + createEnrollmentsForOrder.
  ...
  function renderPaymentConfirmation(orderNumber, email, orderId, pendingOrder) {
    ...
    // Wallet consumption, loyalty, pass provisioning, student records, and
    // enrollments are all handled server-side — submitOrder for $0 orders,
    // squareWebhook / stripeWebhook for paid orders. No client-side writes needed.
