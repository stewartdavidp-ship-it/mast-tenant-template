...
(function() {
  ...
  // viewOrder relocated to shared/orders-core.js (eager) in T6 PR3b. It routes
  // every external order open (fulfillment "View", customers, the dashboard cards,
  // history deep-links, the MastNavStack restorer) through orders-v2 — the sole
  // order surface — so it must resolve even when this module is unloaded. The
  // legacy-detail fallback that lived here was dropped with the V1 detail UI.
  ...
  // ============================================================
  // Shipping Modal (Shippo rate-shop + manual) — RELOCATED
  // ------------------------------------------------------------
  // The full Shippo shipping-modal flow (openShippingModal + _initShippingPanel /
  // _renderManualShipping / _renderShippingPanel / _renderRateResults / shippingSelect*
  // / shippingGetRates / shippingBuyLabel / _offerLabelPrint / printShippingLabel /
  // shippingVoidLabel / submitManualShipping + the _shippingState/_getShippingProvider/
  // _getStudioLocations/_getPackagePresets/_calcOrderWeight helpers) was relocated
  // VERBATIM to fulfillment.js in T6 PR3b. fulfillment.js's pack/ship surface is its
  // only consumer (the "Label" onclick); keeping it lazy there avoids bloating the
  // eager head. The order-detail Simple Ship dialog (openSimpleShipDialog/
  // submitSimpleShip) below is NOT Shippo and stays with the legacy detail UI.
  // ============================================================
  // ============================================================
  ...
  // viewOrder relocated to shared/orders-core.js (eager) in T6 PR3b — it is
  // broadly shared (shell history deep-links, fulfillment.js, customers.js, the
  // dashboard cards, the RMA banner) and must resolve with orders.js unloaded.
  ...
  // The Shippo shipping-modal flow (openShippingModal + submitManualShipping +
  // shippingGetRates/BuyLabel/SelectRate/SelectPreset/BackToConfigure/SwitchToManual
  // + shippingVoidLabel + printShippingLabel and their private helpers) was
  // relocated VERBATIM to fulfillment.js in T6 PR3b — that module is its only
  // consumer (the pack/ship "Label" onclick) and keeping it lazy avoids bloating
  // the eager head. openSimpleShipDialog/submitSimpleShip (the order-detail simple
  // ship dialog) stay here with the legacy detail UI (removed in PR3b-2).
