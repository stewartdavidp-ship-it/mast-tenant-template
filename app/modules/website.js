(function() {
  ...
  function renderManifestStyleTab() {
    ...
    templateManifest.colorSchemes.forEach(function(scheme) {
      ...
      var c = scheme.colors;
      html += '<div class="wp-scheme-card' + (isActive ? ' selected' : '') + '" onclick="wpSelectScheme(\'' + esc(scheme.id) + '\')" style="cursor:pointer;padding:12px;border-radius:10px;border:2px solid ' + (isActive ? 'var(--accent)' : 'var(--charcoal-light, #333)') + ';background:var(--surface-card);transition:border-color 0.2s;">';
      // Color circles row
    ...
    // Custom option card
    html += '<div class="wp-scheme-card' + (showCustomColors ? ' selected' : '') + '" onclick="wpSelectSchemeCustom()" style="cursor:pointer;padding:12px;border-radius:10px;border:2px solid ' + (showCustomColors ? 'var(--accent)' : 'var(--charcoal-light, #333)') + ';background:var(--surface-card);transition:border-color 0.2s;">';
    html += '<div style="display:flex;gap:6px;margin-bottom:8px;">';
    ...
    templateManifest.fontPairs.forEach(function(fp) {
      ...
      html += '<div class="wp-font-card' + (isActive ? ' selected' : '') + '" onclick="wpSelectFontPair(\'' + esc(fp.id) + '\')" style="cursor:pointer;padding:14px;border-radius:10px;border:2px solid ' + (isActive ? 'var(--accent)' : 'var(--charcoal-light, #333)') + ';background:var(--surface-card);transition:border-color 0.2s;">';
  ...
  function renderCustomColorPickers() {
    ...
    var html = '';
    html += '<div style="margin-top:12px;padding:16px;background:var(--surface-card);border-radius:10px;border:1px solid var(--charcoal-light, #333);">';
  ...
  MastAdmin.registerModule('website', {
    routes: {
      'website': { tab: 'websiteTab', setup: function() {
        // The cross-module website WRITE layer (WebsiteBridge + the destructive
        // template-switch cascade wpConfirmSwitch + the category manager dialog
        // wpCat*/openManageCategoriesDialog + markUnpublished) now lives in the
        // lazy website-core.js. Ensure it's loaded so this V1 UI's onclick window
        // globals resolve in legacy mode (the only place this V1 UI is reached).
        // website-core is a no-op non-routed core.
