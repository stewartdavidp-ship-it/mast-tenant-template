...
(function () {
  ...
  MastEntity.define('resources-v2', {
    ...
    detail: {
      render: function (UI, r) {
        ...
        var dangerZone = can('resources', 'delete')
          ? UI.card('Danger zone', '<button class="btn btn-danger btn-small" onclick="ResourcesV2.remove(\'' + esc(r._key || r.id) + '\')">Delete resource</button>')
          : '';
  ...
  function loadData() {
    ...
    return Promise.all([
      ...
      V2.loaded = true;
    }).catch(function (e) { console.error('[resources-v2] load', e); });
  }
  ...
  window.ResourcesV2 = {
    remove: function (id) {
      if (!can('resources', 'delete')) { if (window.showToast) showToast('Delete access required.', true); return; }
      if (!window.ResourcesBridge || !window.ResourcesBridge.remove) { if (window.showToast) showToast('Engine still loading — try again', true); return; }
      var rec = V2.byId[id];
      var refs = rec ? V2.classes.filter(function (c) { return c && (c.resourceId === id || (rec.name && c.resourceName === rec.name)); }).length : 0;
      var msg = 'Delete "' + ((rec && rec.name) || '') + '"?' + (refs ? ' ' + refs + ' class' + (refs === 1 ? ' books' : 'es book') + ' this resource — they keep the name but lose the link.' : '') + ' This cannot be undone.';
      mastConfirm(msg, { title: 'Delete Resource', confirmLabel: 'Delete', danger: true }).then(function (ok) {
        if (!ok) return;
        Promise.resolve(window.ResourcesBridge.remove(id)).then(function () {
          delete V2.byId[id];
          V2.rows = V2.rows.filter(function (x) { return (x._key || x.id) !== id; });
          if (window.showToast) showToast('Resource deleted');
